...
defmodule Solid.Context do
  ...
  def run_cycle(%__MODULE__{cycle_state: cycle_state} = context, cycle) do
    ...
    case cycle_state[name] do
      {current_index, cycle_map} ->
        ...
        {%{context | cycle_state: %{context.cycle_state | name => {next_index, cycle_map}}}, cycle_map[next_index]}
      ...
      nil ->
        ...
        {%{context | cycle_state: Map.put_new(cycle_state, name, {current_index, cycle_map})}, cycle_map[current_index]}
    end
  end
  ...
  defp do_get_in(data, ["first" | keys]) when is_list(data) do
    data |> List.first() |> Solid.Utils.apply_lazy() |> do_get_in(keys)
  end

  defp do_get_in(data, ["first" | keys]) when is_binary(data) do
    data |> String.first() |> do_get_in(keys)
  end

  defp do_get_in(data, ["last" | keys]) when is_list(data) do
    data |> List.last() |> Solid.Utils.apply_lazy() |> do_get_in(keys)
  end

  defp do_get_in(data, ["last" | keys]) when is_binary(data) do
    data |> String.last() |> do_get_in(keys)
  end
